...
export async function POST(request: NextRequest) {
  try {
    ...
    if (order.paypalOrderId && order.paypalOrderId !== paypalOrderId) {
      console.error(
        `PayPal Order ID mismatch for order ${orderId}. Stored: ${order.paypalOrderId}, Received: ${paypalOrderId}`,
      );
      return NextResponse.json(
        {
          error:
            "PayPal Order ID mismatch. This order is associated with a different PayPal transaction.",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      const paypalAmount = parseFloat(
        paypalOrderDetails.purchase_units[0].amount.value,
      );
      if (Math.abs(paypalAmount - order.totalAmount) > 0.01) {
        console.error(
          `Amount mismatch for order ${orderId}. Expected: ${order.totalAmount}, Received: ${paypalAmount}`,
        );
        return NextResponse.json(
          {
      ...
      if (paypalOrderDetails.status !== "APPROVED") {
        console.error(
          `Invalid PayPal order status: ${paypalOrderDetails.status} for order ${orderId}`,
        );
        return NextResponse.json(
          {
      ...
      throw new Error(
        `Failed to verify PayPal order: ${paypalError instanceof Error ? paypalError.message : "Unknown error"}`,
      );
    }
    ...
    const captureData = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("PayPal capture timeout after 15 seconds")),
          15000,
        ),
      ),
    ]);
    ...
    if (!captureData || captureData.status !== "COMPLETED") {
      ...
      console.error(
        `Payment capture failed for order ${orderId}. Status: ${captureData?.status || "NO_RESPONSE"}`,
      );
    ...
    console.log(
      `Payment captured successfully for order ${orderId}. Capture ID: ${captureData.id}`,
    );
    ...
    try {
      ...
      console.error(
        "Failed to update order status after critical error:",
        updateError,
      );
    }
